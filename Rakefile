...
namespace :test do
  %w(active_record_50).each do |version|
    desc "Test acts_as_paranoid against #{version}"
    task version do
      sh "env BUNDLE_GEMFILE='gemfiles/#{version}.gemfile' bundle install --quiet"
      sh "env BUNDLE_GEMFILE='gemfiles/#{version}.gemfile' bundle exec rake -t test"
    end
  end
